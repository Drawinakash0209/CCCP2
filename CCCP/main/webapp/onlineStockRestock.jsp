    ...
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-white dark:bg-gray-700 text-black dark:text-white">
        ...
        <div class="h-full ml-14 mt-14 mb-10 md:ml-64 px-4">
            <div class="max-w-md mx-auto bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-10">
                ...
                <form id="restock-online-form" class="space-y-4" @submit.prevent="restockOnline">
                    <div>
...
<script>
    const setup = () => {
        ...
        return {
            ...
            productId: '',
            quantity: '',
            lastRestock: null, // Track the last restock event to avoid duplicates
            async restockOnline() {
                const formData = new FormData(document.getElementById('restock-online-form'));
                ...
                try {
                    ...
                    setTimeout(() => {
                        if (messageDiv.textContent.includes('Failed to restock')) {
                            messageDiv.classList.add('hidden');
                        }
                    }, 3000);
                }
            ...
            async checkRestockEvents() {
                try {
                    const response = await fetch('/CCCP/OnlineOrderServlet?checkRestock=true&t=' + Date.now(), {
                        method: 'GET',
                        headers: {
                    ...
                    if (event && event.timestamp && (!this.lastRestock || this.lastRestock !== event.timestamp)) {
                        const messageDiv = document.getElementById('message');
                        messageDiv.classList.remove('hidden', 'bg-red-100', 'text-red-700', 'bg-green-100', 'text-green-700');
                        messageDiv.textContent = `Online inventory restocked: Product ID ${event.productId} with ${event.quantity} units`;
                        messageDiv.classList.add('bg-green-100', 'text-green-700');
                        this.lastRestock = event.timestamp;
                        setTimeout(() => {
                            if (messageDiv.textContent.includes('Online inventory restocked')) {
                                messageDiv.classList.add('hidden');
                            }
                        }, 3000);
                    }
                    ...
                    if (error.message.includes('401')) {
                        window.location.href = 'login.jsp?error=Session expired. Please login again.';
                    }
                }
            }
        };
